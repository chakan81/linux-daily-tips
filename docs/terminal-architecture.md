...
## 🏃 6. 프로토타입 구현 가이드 (Phase 2 Week 3)

### 6.1 프로토타입 범위 정의

**포함 기능 (최소 기능)**
- ✅ 단일 세션 생성 및 종료
- ✅ 기본 명령어 실행 (ls, pwd, echo, cat)
- ✅ WebSocket 실시간 통신
- ✅ 단순 에러 처리 (타임아웃, 연결 끊김)
- ✅ 30초 세션 타임아웃
...
**체크리스트**
- [ ] xterm.js 설치 (`npm install @xterm/xterm @xterm/addon-fit`)
- [ ] TerminalEmulator.tsx 컴포넌트 작성
- [ ] shadcn/ui Card로 터미널 래핑
- [ ] 기본 입력/출력 테스트 (로컬 에코)
...
**체크리스트**
- [ ] FastAPI WebSocket 엔드포인트 구현 (`/ws/terminal/{session_id}`)
- [ ] useTerminalWebSocket.ts 커스텀 훅 작성
- [ ] 메시지 송수신 테스트 (echo 서버)
...
**체크리스트**
- [ ] Docker Python SDK 설치 (pyproject.toml에 `docker>=7.1.0` 추가 후 `docker-compose build backend`)
- [ ] Dockerfile 작성 (Ubuntu 24.04 기반)
- [ ] TerminalService 구현 (컨테이너 생명주기 관리)
- [ ] exec_run 비동기 래퍼 작성
- [ ] 세션 생성 API 구현 (`GET /api/v1/terminal/session`)
...
**체크리스트**
- [ ] 네트워크 격리 적용 (`network_mode="none"`)
- [ ] 리소스 제한 적용 (mem_limit, cpu_quota)
- [ ] 세션 타임아웃 구현 (Redis TTL 30초)
- [ ] 컨테이너 정리 크론잡 구현
- [ ] 응답 시간 측정 및 최적화 (< 2초 목표)
...
**응답 시간 측정**
- [ ] 세션 생성: < 2초 (브라우저 네트워크 탭 확인)
- [ ] 명령어 실행 (ls): < 1초 (WebSocket 왕복 시간)
- [ ] 명령어 실행 (pwd): < 800ms
- [ ] WebSocket 지연: < 100ms (ping/pong 테스트)

**리소스 사용량 측정**
- [ ] 컨테이너 메모리: < 256MB (docker stats 확인)
- [ ] 컨테이너 CPU: < 50% (docker stats 확인)
- [ ] 프론트엔드 메모리: < 100MB (브라우저 개발자 도구)

**동시 세션 테스트**
- [ ] 5개 동시 세션 생성 및 실행 (동시성 기본 검증)
- [ ] 10개 동시 세션 (목표: 모두 2초 이내 생성)

---

## 📝 7. 구현 단계별 상세 계획

### 7.1 Phase 2 (프로토타입 - Week 3)
...
**주요 작업**
1. xterm.js 터미널 UI 구현
2. WebSocket 실시간 통신
3. Docker 컨테이너 명령어 실행
4. 기본 보안 및 타임아웃

**완료 기준**
- ls, pwd, echo 명령어 정상 실행
- 30초 세션 타임아웃 동작
- 단순 에러 처리 (연결 끊김, 타임아웃)

### 7.2 Phase 3 (검증 - Week 4 Day 22-23)

**목표**: 실제 사용자 시나리오 테스트
**기간**: 2일
**검증 항목**: 전체 기능 통합, 성능 측정
...
### 7.3 Phase 4 (확장 - Phase 2 프로젝트)

**목표**: 프로덕션 준비 기능 추가
**기간**: Phase 2 프로젝트 (3주) 중 1주
**확장 기능**: 보안 강화, 성능 최적화, 반응형 UI
