...
**현재 동작 플로우**:
```
사용자 입력 (Enter) → WebSocket 전송 → Docker exec 실행 → 결과 반환 → 터미널 출력
```
...
#### Day 21: 보안 및 최적화 ⚠️ (부분 완료)
**목표**: 기본 보안 설정 및 성능 최적화

**체크리스트**
- [x] 네트워크 격리 적용 (`network_disabled=True`) - 코드 작성 완료
- [x] 리소스 제한 적용 (mem_limit=256m, cpu_quota=0.5) - 코드 작성 완료
- [x] 세션 타임아웃 구현 (30분 자동 만료) - 코드 작성 완료
- [ ] 컨테이너 정리 크론잡 구현 - 미완성
- [ ] 응답 시간 측정 및 최적화 (< 2초 목표) - 브라우저 검증 필요

**완료 기준**: 보안 설정 적용 확인, < 2초 응답 시간 달성 ⏳
**현재 상태**: 보안 코드 작성 완료, 실제 동작 검증 필요
...
### 7.1 Phase 1 (PoC 프로토타입 - Week 3) ✅ (75% 완료)
...
4. ✅ 기본 보안 설정 (네트워크 격리, 리소스 제한)
5. ✅ 세션 타임아웃 구현 (30분 자동 만료)
6. ✅ Playwright 자동화 테스트
